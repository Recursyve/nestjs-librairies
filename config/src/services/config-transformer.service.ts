...
export class ConfigTransformerService {
    ...
    public async transform(configType: Type): Promise<Record<string, any>> {
        ...
        const configProvider = this.configProviderRegistry.getConfigProvider(
        ...
        if (undefinedVariableNames.length) {
            if (process.env.NODE_ENV === "test" || !!process.env.CI) {
                console.log(
                    `The following variable must be defined in provider '${
                        configProvider.constructor.name
                    }': ${JSON.stringify(undefinedVariableNames)}. Skipping for test mode.`
                );
            } else {
                throw new Error(
                    `The following variable must be defined in provider '${
                        configProvider.constructor.name
                    }': ${JSON.stringify(undefinedVariableNames)}`
                );
            }
        }
