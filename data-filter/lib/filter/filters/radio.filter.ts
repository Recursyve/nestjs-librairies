...
export class RadioFilter extends Filter implements RadioFilterDefinition {
    ...
    public async getConfig<Request>(key: string, req: Request, user?: DataFilterUserModel): Promise<RadioFilterConfigurationModel> {
        if (this.private) {
            return;
        }

        return {
            ...(await super.getConfig(key, req, user)),
